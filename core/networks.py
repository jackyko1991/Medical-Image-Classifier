...
class InceptionNet2D(object):
	...
	def inception_module(self, 
		...
		with tf.variable_scope(name):
			if channels_1x1 > 1:
				...
				conv_1x1 = self.ConvActivate2d_block(input_tensor, [1,1,input_channels,channels_1x1],strides=strides,padding='SAME',name="1x1",is_training=self.is_training)
				filters.append(conv_1x1)

			if channels_3x3 >1:
				conv_3x3_reduce = self.ConvActivate2d_block(input_tensor, [1,1,input_channels,channels_3x3_reduce],padding='SAME',name="3x3_reduce",is_training=self.is_training)
				if linear_factorization ==False and wide_module == False:
					conv_3x3 = self.ConvActivate2d_block(conv_3x3_reduce, [3,3,channels_3x3_reduce,channels_3x3],strides=strides,padding='SAME',name="3x3",is_training=self.is_training)
					filters.append(conv_3x3)
				elif linear_factorization == True:
					conv_3x3 = self.ConvActivate2d_block(conv_3x3_reduce, [1,linear_kernel_size,channels_3x3_reduce,channels_3x3],padding='SAME',name="3x3_1",is_training=self.is_training)
					conv_3x3 = self.ConvActivate2d_block(conv_3x3, [linear_kernel_size,1,channels_3x3,channels_3x3],padding='SAME',name="3x3_2",is_training=self.is_training)
					filters.append(conv_3x3)
				elif wide_module == True:
					conv_3x3_1 = self.ConvActivate2d_block(conv_3x3_reduce, [1,linear_kernel_size,channels_3x3_reduce,channels_3x3],padding='SAME',name="3x3_1",is_training=self.is_training)
					conv_3x3_2 = self.ConvActivate2d_block(conv_3x3_reduce, [linear_kernel_size,1,channels_3x3_reduce,channels_3x3],padding='SAME',name="3x3_2",is_training=self.is_training)
					filters.extend([conv_3x3_1,conv_3x3_2])
			
			if channels_5x5 > 1:
				conv_5x5_reduce = self.ConvActivate2d_block(input_tensor, [1,1,input_channels,channels_5x5_reduce],padding='SAME',name="5x5_reduce",is_training=self.is_training)
				if version == 1:
					conv_5x5 = self.ConvActivate2d_block(conv_5x5_reduce, [5,5,channels_5x5_reduce,channels_5x5],padding='SAME',name="5x5",is_training=self.is_training)
					filters.append(conv_5x5)
				elif version >1 and linear_factorization == False and wide_module == False:
					conv_5x5 = self.ConvActivate2d_block(conv_5x5_reduce, [3,3,channels_5x5_reduce,channels_5x5],padding='SAME',name="5x5_1",is_training=self.is_training)
					conv_5x5 = self.ConvActivate2d_block(conv_5x5, [3,3,channels_5x5,channels_5x5],strides=strides,padding='SAME',name="5x5_2",is_training=self.is_training)
					filters.append(conv_5x5)
				elif version >1 and linear_factorization == True:
					conv_5x5 = self.ConvActivate2d_block(conv_5x5_reduce, [1,linear_kernel_size,channels_5x5_reduce,channels_5x5],padding='SAME',name="5x5_1",is_training=self.is_training)
					conv_5x5 = self.ConvActivate2d_block(conv_5x5, [linear_kernel_size,1,channels_5x5,channels_5x5],padding='SAME',name="5x5_2",is_training=self.is_training)
					conv_5x5 = self.ConvActivate2d_block(conv_5x5, [1,linear_kernel_size,channels_5x5,channels_5x5],strides=strides,padding='SAME',name="5x5_3",is_training=self.is_training)
					conv_5x5 = self.ConvActivate2d_block(conv_5x5, [linear_kernel_size,1,channels_5x5,channels_5x5],strides=strides,padding='SAME',name="5x5_4",is_training=self.is_training)
					filters.append(conv_5x5)
				elif version >1 and version <4 and wide_module == True:
					conv_5x5_1 = self.ConvActivate2d_block(conv_5x5_reduce, [3,3,channels_5x5_reduce,channels_5x5],padding='SAME',name="5x5_1",is_training=self.is_training)
					conv_5x5_2 = self.ConvActivate2d_block(conv_5x5_1, [linear_kernel_size,1,channels_5x5,channels_5x5],padding='SAME',name="5x5_2",is_training=self.is_training)
					conv_5x5_3 = self.ConvActivate2d_block(conv_5x5_1, [1,linear_kernel_size,channels_5x5,channels_5x5],strides=strides,padding='SAME',name="5x5_3",is_training=self.is_training)
					filters.extend([conv_5x5_2,conv_5x5_3])
				elif version ==4 and wide_module == True:
					conv_5x5_1 = self.ConvActivate2d_block(conv_5x5_reduce, [1,3,channels_5x5_reduce,int((channels_5x5_reduce+channels_5x5*2)/2)],padding='SAME',name="5x5_1",is_training=self.is_training)
					conv_5x5_2 = self.ConvActivate2d_block(conv_5x5_1, [3,1,int((channels_5x5_reduce+channels_5x5*2)/2),channels_5x5*2],padding='SAME',name="5x5_2",is_training=self.is_training)
					conv_5x5_3 = self.ConvActivate2d_block(conv_5x5_2, [linear_kernel_size,1,channels_5x5*2,channels_5x5],padding='SAME',name="5x5_3",is_training=self.is_training)
	...
	def reductionA_module(self,
		...
		with tf.variable_scope("reductionA"):
			conv_3x3 = self.ConvActivate2d_block(input_image, [3,3,input_channels,channels_3x3],strides=[1,2,2,1],padding='VALID',name="3x3",is_training=self.is_training)
			filters.append(conv_3x3)

			conv_5x5_reduce = self.ConvActivate2d_block(input_image, [1,1,input_channels,channels_5x5_reduce_1],padding='SAME',name="5x5_reduce_1",is_training=self.is_training)
			conv_5x5_reduce = self.ConvActivate2d_block(conv_5x5_reduce, [3,3,channels_5x5_reduce_1,channels_5x5_reduce_2],padding='SAME',name="5x5_reduce_2",is_training=self.is_training)
			conv_5x5 = self.ConvActivate2d_block(conv_5x5_reduce, [3,3,channels_5x5_reduce_2,channels_5x5],strides=[1,2,2,1],padding='VALID',name="5x5",is_training=self.is_training)
			filters.append(conv_5x5)
	...
	def reductionB_module(self,
		...
		with tf.variable_scope("reductionB"):
			if version == 0:
				conv_3x3_reduce = self.ConvActivate2d_block(input_image, [1,1,input_channels,192],strides=[1,1,1,1],padding='SAME',name="3x3_1",is_training=self.is_training)
				conv_3x3 = self.ConvActivate2d_block(conv_3x3_reduce, [3,3,192,192],strides=[1,2,2,1],padding='VALID',name="3x3_2",is_training=self.is_training)
				filters.append(conv_3x3)
			elif version == 1:
				conv_3x3_reduce_1 = self.ConvActivate2d_block(input_image, [1,1,input_channels,256],strides=[1,1,1,1],padding='SAME',name="3x3_1",is_training=self.is_training)
				conv_3x3_1 = self.ConvActivate2d_block(conv_3x3_reduce_1, [3,3,256,384],strides=[1,2,2,1],padding='VALID',name="3x3_2",is_training=self.is_training)
				filters.append(conv_3x3_1)
				conv_3x3_reduce_2 = self.ConvActivate2d_block(input_image, [1,1,input_channels,256],strides=[1,1,1,1],padding='SAME',name="3x3_1",is_training=self.is_training)
				conv_3x3_2 = self.ConvActivate2d_block(conv_3x3_reduce_2, [3,3,256,256],strides=[1,2,2,1],padding='VALID',name="3x3_2",is_training=self.is_training)
				filters.append(conv_3x3_2)
			elif version == 2:
				conv_3x3_reduce_1 = self.ConvActivate2d_block(input_image, [1,1,input_channels,256],strides=[1,1,1,1],padding='SAME',name="3x3_1_1",is_training=self.is_training)
				conv_3x3_1 = self.ConvActivate2d_block(conv_3x3_reduce_1, [3,3,256,384],strides=[1,2,2,1],padding='VALID',name="3x3_1_2",is_training=self.is_training)
				filters.append(conv_3x3_1)
				conv_3x3_reduce_2 = self.ConvActivate2d_block(input_image, [1,1,input_channels,256],strides=[1,1,1,1],padding='SAME',name="3x3_2_1",is_training=self.is_training)
				conv_3x3_2 = self.ConvActivate2d_block(conv_3x3_reduce_2, [3,3,256,256],strides=[1,2,2,1],padding='VALID',name="3x3_2_2",is_training=self.is_training)
				filters.append(conv_3x3_2)
				
			conv_5x5_reduce = self.ConvActivate2d_block(input_image, [1,1,input_channels,256],padding='SAME',name="5x5_reduce_1",is_training=self.is_training)
			if version == 0:
				conv_5x5_reduce = self.ConvActivate2d_block(conv_5x5_reduce, [1,7,256,256],padding='SAME',name="5x5_reduce_2",is_training=self.is_training)
				conv_5x5_reduce = self.ConvActivate2d_block(conv_5x5_reduce, [7,1,256,320],padding='SAME',name="5x5_reduce_3",is_training=self.is_training)
				conv_5x5 = self.ConvActivate2d_block(conv_5x5_reduce, [3,3,320,320],strides=[1,2,2,1],padding='VALID',name="5x5",is_training=self.is_training)
			elif version == 1 or version == 2:
				conv_5x5 = self.ConvActivate2d_block(conv_5x5_reduce, [3,3,256,256],padding='SAME',name="5x5_2",is_training=self.is_training)
				conv_5x5 = self.ConvActivate2d_block(conv_5x5, [3,3,256,256],strides=[1,2,2,1],padding='VALID',name="5x5_3",is_training=self.is_training)
			# elif version == 2:
			# 	conv_5x5 = self.ConvActivate2d_block(conv_5x5_reduce, [3,3,256,256],padding='SAME',name="5x5_2",is_training=self.is_training)
			# 	conv_5x5 = self.ConvActivate2d_block(conv_5x5, [3,3,256,256],strides=[1,2,2,1],padding='VALID',name="5x5_3",is_training=self.is_training)
			filters.append(conv_5x5)
	...
	def inception_residual_module_A(self, input_image, name ="", dropout=0.0, version=1, scale=1.0):
		"""
		inception residual module A
		"""
		...
		with tf.variable_scope(name):
			conv_1x1 = self.ConvActivate2d_block(input_image, [1,1,input_channels,32],padding='SAME',name="1x1",is_training=self.is_training)
			filters.append(conv_1x1)

			conv_3x3_reduce = self.ConvActivate2d_block(input_image, [1,1,input_channels,32],padding='SAME',name="3x3_reduce",is_training=self.is_training)
			conv_3x3 = self.ConvActivate2d_block(conv_3x3_reduce, [3,3,32,32],padding='SAME',name="3x3",is_training=self.is_training)
			filters.append(conv_3x3)
			
			conv_5x5_reduce = self.ConvActivate2d_block(input_image, [1,1,input_channels,32],padding='SAME',name="5x5_reduce",is_training=self.is_training)
			if version == 1:
				conv_5x5 = self.ConvActivate2d_block(conv_5x5_reduce, [3,3,32,32],padding='SAME',name="5x5_1",is_training=self.is_training)
				conv_5x5 = self.ConvActivate2d_block(conv_5x5, [3,3,32,32],strides=strides,padding='SAME',name="5x5_2",is_training=self.is_training)
			elif version == 2:
				conv_5x5 = self.ConvActivate2d_block(conv_5x5_reduce, [3,3,32,48],padding='SAME',name="5x5_1",is_training=self.is_training)
				conv_5x5 = self.ConvActivate2d_block(conv_5x5, [3,3,48,64],padding='SAME',name="5x5_2",is_training=self.is_training)
			filters.append(conv_5x5)

			x = tf.concat(filters,axis=-1)

			if version == 1:
				output = self.ConvActivate2d_block(x, [1,1,int(x.get_shape()[-1]),256],padding='SAME',name="output",is_training=self.is_training)
			elif version == 2:
				output = self.ConvActivate2d_block(x, [1,1,int(x.get_shape()[-1]),384],padding='SAME',name="output",is_training=self.is_training)

			output = input_image + scale*output

		return output

	def inception_residual_module_B(self, input_image, name ="", dropout=0.0, version=1, scale=1.0):
		"""
		inception residual module B
		"""
		...
		with tf.variable_scope(name):
			if version == 1:
				conv_1x1 = self.ConvActivate2d_block(input_image, [1,1,input_channels,128],padding='SAME',name="1x1",is_training=self.is_training)
			elif version == 2:
				conv_1x1 = self.ConvActivate2d_block(input_image, [1,1,input_channels,192],padding='SAME',name="1x1",is_training=self.is_training)
			filters.append(conv_1x1)
			
			conv_5x5_reduce = self.ConvActivate2d_block(input_image, [1,1,input_channels,128],padding='SAME',name="7x7_reduce",is_training=self.is_training)
			if version == 1:
				conv_5x5 = self.ConvActivate2d_block(conv_5x5_reduce, [1,7,128,128],padding='SAME',name="7x7_1",is_training=self.is_training)
				conv_5x5 = self.ConvActivate2d_block(conv_5x5, [7,1,128,128],padding='SAME',name="7x7_2",is_training=self.is_training)
			elif version == 2:
				conv_5x5 = self.ConvActivate2d_block(conv_5x5_reduce, [1,7,128,160],padding='SAME',name="7x7_1",is_training=self.is_training)
				conv_5x5 = self.ConvActivate2d_block(conv_5x5, [7,1,160,192],padding='SAME',name="7x7_2",is_training=self.is_training)
			filters.append(conv_5x5)

			x = tf.concat(filters,axis=-1)

			if version == 1:
				output = self.ConvActivate2d_block(x, [1,1,int(x.get_shape()[-1]),896],padding='SAME',name="output",is_training=self.is_training)
			elif version == 2:
				output = self.ConvActivate2d_block(x, [1,1,int(x.get_shape()[-1]),1152],padding='SAME',name="output",is_training=self.is_training)

			output = input_image + scale*output

		return output

	def inception_residual_module_C(self, input_image, name ="", dropout=0.0, version=1, scale=1.0):
		"""
		inception residual module C
		"""
		...
		with tf.variable_scope(name):
			...
			conv_3x3_reduce = self.ConvActivate2d_block(input_image, [1,1,input_channels,192],padding='SAME',name="3x3_reduce",is_training=self.is_training)
			if version == 1:
				conv_3x3 = self.ConvActivate2d_block(conv_3x3_reduce, [1,3,192,192],padding='SAME',name="3x3_1",is_training=self.is_training)
				conv_3x3 = self.ConvActivate2d_block(conv_5x5, [3,1,192,192],padding='SAME',name="3x3_2",is_training=self.is_training)
			elif version == 2:
				conv_3x3 = self.ConvActivate2d_block(conv_3x3_reduce, [1,3,192,224],padding='SAME',name="3x3_1",is_training=self.is_training)
				conv_3x3 = self.ConvActivate2d_block(conv_3x3, [3,1,224,256],padding='SAME',name="3x3_2",is_training=self.is_training)
			filters.append(conv_3x3)

			x = tf.concat(filters,axis=-1)

			if version == 1:
				output = self.ConvActivate2d_block(x, [1,1,int(x.get_shape()[-1]),1792],padding='SAME',name="output",is_training=self.is_training)
			elif version == 2:
				output = self.ConvActivate2d_block(x, [1,1,int(x.get_shape()[-1]),2048],padding='SAME',name="output",is_training=self.is_training)
			output = input_image + scale*output

		return output

	def auxillary_output(self,input_image, dropout=0.0,units=1024,name="",batch_norm=False):
		with tf.variable_scope(name):
			x = tf.nn.avg_pool(input_image,[1,5,5,1],strides=[1,3,3,1],padding='VALID')
			x = self.ConvActivate2d_block(x,[1,1,int(input_image.get_shape()[-1]),128],padding='VALID',is_training=self.is_training)
	...
	def InceptionV1(self,input_image,dropout=0.0):
		...
		with tf.variable_scope('block4'):
			...
			aux_1 = self.auxillary_output(x,dropout=dropout,name="auxillary_output_1")
			aux.append(aux_1)
			...
			aux_2 = self.auxillary_output(x,dropout=dropout,name="auxillary_output_2")
			aux.append(aux_2)
	...
	def InceptionV2(self, input_image,dropout=0.0):
		"""
	...
	def InceptionV3(self, input_image,dropout=0.0):
		"""
		...
		with tf.variable_scope('block4'):
			...
			aux_1 = self.auxillary_output(x,dropout=dropout,name="auxillary_output_1",batch_norm=True)
			aux.append(aux_1)
			"""
	...
	def InceptionV4(self, input_image,dropout=0.0):
		"""
		Inception V4
		"""
		input_channels = int(input_image.get_shape()[-1])
		...
		with tf.variable_scope('stem'):
			...
			x = tf.nn.local_response_normalization(x, depth_radius=2, alpha=2e-05, beta=0.75,name='pool1_lrn')
			# block 2 branch 1
			x_1 = self.ConvActivate2d_block(x,[1,1,160,64],padding='SAME',is_training=self.is_training, name="conv_2_1a")
			x_1 = self.ConvActivate2d_block(x_1,[3,3,64,96],padding='VALID',is_training=self.is_training, name="conv_2_1b")

			# block 2 branch 2
			x_2 = self.ConvActivate2d_block(x,[1,1,160,64],padding='SAME',is_training=self.is_training, name="conv_2_2a")
			x_2 = self.ConvActivate2d_block(x_2,[7,1,64,64],padding='SAME',is_training=self.is_training, name="conv_2_2b")
			x_2 = self.ConvActivate2d_block(x_2,[1,7,64,64],padding='SAME',is_training=self.is_training, name="conv_2_2c")
			x_2 = self.ConvActivate2d_block(x_2,[3,3,64,96],padding='VALID',is_training=self.is_training, name="conv_2_2d")

			x = tf.concat([x_1,x_2],axis=-1)

			x_1 = tf.nn.max_pool2d(x, ksize=[1,3,3,1], strides=[1,2,2,1], padding = 'VALID',name='pool_2e')
			x_2 = self.ConvActivate2d_block(x,[3,3,192,192],strides=[1,2,2,1],padding='VALID',name="conv_2e")
			x = tf.concat([x_1,x_2],axis=-1)
		...
		x = self.reductionB_module(x,version=0)
	...
	def InceptionResNetV1(self,input_image,dropout=0.0):
		"""
		Inception ResNet V1
		"""
		input_channels = int(input_image.get_shape()[-1])
		...
		with tf.variable_scope('stem'):
			...
			x = self.ConvActivate2d_block(x,[3,3,32,32],strides=[1,1,1,1],padding='VALID',is_training=self.is_training, name="conv_1b")
			x = self.ConvActivate2d_block(x,[3,3,32,64],strides=[1,1,1,1],padding='SAME',is_training=self.is_training, name="conv_1c")
			x = tf.nn.max_pool2d(x, ksize=[1,3,3,1], strides=[1,2,2,1], padding = 'VALID',name='pool_1')

			# block 2 branch 1
			x = self.ConvActivate2d_block(x,[1,1,64,80],padding='SAME',is_training=self.is_training, name="conv_2a")
			x = self.ConvActivate2d_block(x,[3,3,80,192],padding='VALID',is_training=self.is_training, name="conv_2b")
			x = self.ConvActivate2d_block(x,[3,3,192,256],strides=[1,2,2,1],padding='VALID',name="conv_2c")

		# block 3
		for i in range(5):
			x = self.inception_residual_module_A(x, name="inception_residual_A_{}".format(str(i)), dropout=0.0, version=1)
			
		x = self.reductionA_module(x,channels_3x3=384,channels_5x5_reduce_1=192,channels_5x5_reduce_2=192,channels_5x5=256,scale=0.1)

		# block 4
		for i in range(10):
			x = self.inception_residual_module_B(x, name="inception_residual_B_{}".format(str(i)), dropout=0.0, version=1)
			
		x = self.reductionB_module(x,version=1,scale=0.1)

		# block 5
		for i in range(5):
			x = self.inception_residual_module_C(x, name="inception_residual_C_{}".format(str(i)), dropout=0.0, version=1)

		# output
		with tf.variable_scope('output'):
			output = tf.nn.avg_pool(x,[1,5,5,1],strides=[1,3,3,1],padding='SAME')
	...
	def InceptionResNetV2(self,input_image,dropout=0.0):
		"""
		Inception ResNet V2
		"""
		input_channels = int(input_image.get_shape()[-1])
		...
		# block 3
		for i in range(5):
			x = self.inception_residual_module_A(x, name="inception_residual_A_{}".format(str(i)), dropout=0.0, version=2,scale=0.1)
		x = self.reductionA_module(x,channels_3x3=384,channels_5x5_reduce_1=256,channels_5x5_reduce_2=256,channels_5x5=384)

		# block 4
		for i in range(10):
			x = self.inception_residual_module_B(x, name="inception_residual_B_{}".format(str(i)), dropout=0.0, version=2,scale=0.1)
		x = self.reductionB_module(x,version=2)

		# block 5
		for i in range(5):
			x = self.inception_residual_module_C(x, name="inception_residual_C_{}".format(str(i)), dropout=0.0, version=2,scale=0.1)
	...
	def GetNetwork(self, input_image):
		# dropout = self.dropout if self.is_training else 0.0
		input_channels = int(input_image.get_shape()[-1])

		with tf.variable_scope('InceptionNet2D'):
			if self.residual == False:
				if self.version == 1:
					output = self.InceptionV1(input_image,dropout=self.dropout)
				elif self.version == 2:
					output = self.InceptionV2(input_image,dropout=self.dropout)
				elif self.version == 3:
					output = self.InceptionV3(input_image,dropout=self.dropout)
				elif self.version == 4:
					output = self.InceptionV4(input_image,dropout=self.dropout)
			else:
				if self.version == 1:
					output = self.InceptionResNetV1(input_image,dropout=self.dropout)
				elif self.version == 2:
					output = self.InceptionResNetV2(input_image,dropout=self.dropout)
...
class Resnet2D(object):
	...
	def residual_shortcut_block(self, input_tensor, channels, is_training=True):
		...
		input_channels = int(input_tensor.get_shape()[-1])
		conv1Filter_shape = [3,3,input_channels,channels]
		conv1 = self.ConvActivate2d_block(input_tensor_padded, conv1Filter_shape, strides = [1,2,2,1], is_training=is_training)
		conv1 = tf.pad(conv1, paddings, "CONSTANT")
		conv2Filter_shape = [3,3,channels,channels]
		conv2 = self.Conv2d_block(conv1, conv2Filter_shape, is_training=is_training)
